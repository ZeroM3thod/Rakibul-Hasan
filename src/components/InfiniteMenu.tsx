...
void main() {
    ...
    // Explicit circular clip for guaranteed roundness
    vec2 distVec = vUvs - 0.5;
...
class ArcballControl {
  ...
  public update(deltaTime: number, targetFrameDuration = 16): void {
    ...
    const RA_INTENSITY = 0.4 * timeScale;
    ...
    const RV_INTENSITY = 0.2 * timeScale;
      ...
      {activeItem && (
        <div
          ...
          className={`
            ...
            border-[4px]
            border-background dark:border-zinc-900
            rounded-full
            cursor-pointer
            shadow-xl
            transition-all
          ...
          `}
        >
          <p className="select-none relative text-white dark:text-black top-[1px] text-[22px] font-bold">&#x2197;</p>
        </div>
      )}
    </div>
  );
